...
class State:
    ...
    grid: chex.Array  # (grid_size, grid_size, grid_size)
    step_count: chex.Array  # ()
    agents: Agent  # (num_agents, ...)
    key: chex.PRNGKey  # (2,) # TODO   
...
class Observation(NamedTuple):
    ...
    action_mask: boolean array representing whether each of the 7 actions is legal, for each agent.
    step_count: (int32) the current episode step.
    """

    grid: chex.Array  # (num_agents, grid_size, grid_size, grid_size)
    action_mask: chex.Array  # (num_agents, 7)
    step_count: chex.Array  # ()
