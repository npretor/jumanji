...
from jumanji.environments.routing.connector_3d.types import Agent
...
def is_valid_position(
    ...
    x_length, y_length, z_length = grid.shape

    # Within the bounds of the grid
    in_bounds = (0 <= row) & (row < x_length) & (0 <= col) & (col < y_length) & (0 <= zloc) & (col < z_length)
    # Cell is not occupied
    open_cell = (grid[row, col, zloc] == EMPTY) | (grid[row, col, zloc] == get_target(agent.id))
...
def connected_or_blocked(agent: Agent, action_mask: chex.Array) -> chex.Array:
    ...
    return agent.connected.all() | jnp.logical_not(action_mask[1:].any()) # TODO action mask will probably not work here
