...
def test_move_position() -> None:
    ...
    pos = jnp.array([1, 1, 0])
    assert (move_position(pos, NOOP) == jnp.array([1, 1, 0])).all()
    assert (move_position(pos, UP) == jnp.array([0, 1, 0])).all()
    assert (move_position(pos, RIGHT) == jnp.array([1, 2, 0])).all()
    assert (move_position(pos, DOWN) == jnp.array([2, 1, 0])).all()
    assert (move_position(pos, LEFT) == jnp.array([1, 0, 0])).all()
    assert (move_position(pos, ZUP) == jnp.array([1, 0, 1])).all()
    assert (move_position(pos, ZDOWN) == jnp.array([1, 0, 0])).all()


def test_move_agent(state: State) -> None:
    """Tests that `move_agent` returns the correct Agent struct."""
    new_position = jnp.array([1, 1, 1])
...
def test_move_agent_invalid(state: State) -> None:
    ...
    invalid_position = jnp.array([1, 1, 1, 1])
    agent0 = tree_slice(state.agents, 0)
...
def test_is_valid_position(state: State) -> None:
    ...
    valid_move = is_valid_position(
        grid=state.grid, agent=agent1, position=jnp.array([2, 2, 0])
    )
    move_into_path = is_valid_position(
        grid=state.grid, agent=agent1, position=jnp.array([4, 2, 0])
    )
...
def test_connected_or_blocked() -> None:
    ...
    not_connected_agent = Agent(
        ...
        start=jnp.array([1, 1, 0]),
        target=jnp.array([1, 3, 0]),
        position=jnp.array([1, 2, 0]),
    )
    connected_agent = Agent(
        id=jnp.array(0, jnp.int32),
        start=jnp.array([1, 2, 0]),
        target=jnp.array([1, 2, 0]),
        position=jnp.array([1, 2, 0]),
    )
